...
async function commitToGitHub({ owner, repo, path, token, content, message }) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (res.status === 200 || res.status === 201) {
      ...
      return { success: true, message: 'Data guru berhasil disimpan!', commitSha: result.commit?.sha };
    }

    if (res.status === 409 && attempt < 3) {
      await new Promise(r => setTimeout(r, 500)); // tunggu 0.5 detik sebelum retry
