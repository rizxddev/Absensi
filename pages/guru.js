...
export default function Guru() {
  ...
  const judul = tipe === 'sekolah' ? 'Rekap Absensi Sekolah' : 'Rekap Absensi Shalat';
  const list = data.absensi[tgl];

  // Cari nama terpanjang biar kolom nama sejajar
  const maxNama = Math.max(...list.map(r => r.nama.length));
  const colNamaWidth = Math.max(maxNama, 20);

  // Hitung total siswa dan yang tidak hadir
  const total = list.length;
  const tidakShalat = tipe === 'shalat' ? list.filter(r => (r.shalat || '').toLowerCase() === 'tidak').length : 0;
  const tidakSekolah = tipe === 'sekolah' ? list.filter(r => (r.sekolah || '').toLowerCase() !== 'hadir').length : 0;

  let teks = `${judul} - Kelas ${data.kelas} (Wali: ${data.wali_kelas})\nTanggal: ${tgl}\n\n`;
  teks += ` No  ${'Nama'.padEnd(colNamaWidth, ' ')} | Status\n`;
  teks += '-'.repeat(colNamaWidth + 15) + '\n';

  list.forEach((r, i) => {
    const namaPadded = r.nama.padEnd(colNamaWidth, ' ');
    if (tipe === 'sekolah') {
      teks += `${String(i + 1).padStart(2, ' ')}  ${namaPadded} | Sekolah: ${r.sekolah || '-'}\n`;
    } else {
      teks += `${String(i + 1).padStart(2, ' ')}  ${namaPadded} | Shalat: ${r.shalat || '-'}\n`;
    }
  });

  teks += `\n${'-'.repeat(colNamaWidth + 15)}\n`;
  teks += `TIDAK SHALAT  : ${tidakShalat} ORANG\n`;
  teks += `TIDAK SEKOLAH : ${tidakSekolah} ORANG\n`;
  teks += `TOTAL SISWA   : ${total} ORANG\n`;
  teks += `${'-'.repeat(colNamaWidth + 15)}\n`;
  teks += `Generated by Sistem Absensi Sekolah\n© 2025 - Created by Rizky`;
