...
export default function Guru() {
  ...
  // Fungsi ambil hasil absensi & siswa (selalu dari API, bukan public)
  const fetchData = async () => {
    try {
      const [siswaData, hasilData] = await Promise.all([
        fetch('/api/getSiswa', { cache: 'no-store' }).then(r => r.json()),
        fetch('/api/getHasil', { cache: 'no-store' }).then(r => r.json())
      ]);

      setData({ ...hasilData, siswa: siswaData.siswa || [] });
      const keys = Object.keys(hasilData.absensi || {}).sort((a, b) => new Date(b) - new Date(a));
      setTanggalList(keys);
      setTanggalPilih(keys[0] || '');
    } catch {
      setData({ absensi: {}, kelas: '', wali_kelas: '', siswa: [] });
    }
  };
  ...
  useEffect(() => {
    ...
    // Auto-refresh tiap 10 detik supaya selalu update
