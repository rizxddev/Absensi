...
export default function Guru() {
  ...
  // Statistik untuk shalat & sekolah
  const totalShalat = tipe === 'shalat'
    ? list.filter(s => s.shalat && s.shalat.toLowerCase() === 'ya').length
    : 0;
  const tidakShalat = tipe === 'shalat'
    ? list.filter(s => s.shalat && s.shalat.toLowerCase() === 'tidak').length
    : 0;
  ...
  list.forEach((r, i) => {
    if (tipe === 'sekolah') {
      ...
      teks += `${i + 1}. ${r.nama} | Shalat: ${r.shalat}${r.tidakSekolah === 'Ya' ? ' | Tidak Sekolah' : ''}\n`;
    }
  });

  // Tambahkan ringkasan di bawah dengan format baru
  teks += `\n---\n`;
  if (tipe === 'shalat') {
    teks += `SHALAT : ${totalShalat} ORANG\n`;
        ...
        <table className="table-auto w-full text-left border border-gray-700">
          <thead>
            <tr className="bg-purple-700 text-white">
              ...
              <th className="px-4 py-2">Tidak Sekolah</th>
            </tr>
          </thead>
          <tbody>
            {tanggalPilihShalat && dataShalat.absensi[tanggalPilihShalat] ? (
              dataShalat.absensi[tanggalPilihShalat].map((r, i) => (
                <tr key={i} className="border-t border-gray-700">
                  ...
                  <td className="px-4 py-2">{r.tidakSekolah || '-'}</td>
                </tr>
              ))
            ) : (
